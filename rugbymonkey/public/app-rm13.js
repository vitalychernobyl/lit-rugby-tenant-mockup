...
import { tenant } from "./data/tenant-rm13.js";
...
function homePage() {
  ...
  const events = state.hasLocation
    ...
    : [...state.events].sort((a, b) => {
      const aPast = phase(a) === "Past";
      const bPast = phase(b) === "Past";
      if (aPast !== bPast) return aPast ? 1 : -1;
      const dateOrder = aPast
        ? String(b.dateSort).localeCompare(String(a.dateSort))
        : String(a.dateSort).localeCompare(String(b.dateSort));
...
function filteredEventsFromUrl(defaultCode = "", defaultStatus = "") {
  ...
  const code = requestedCode === "all" ? "" : requestedCode || defaultCode;
  const region = params.get("region") || "";
  const requestedStatus = params.get("status");
  const status = requestedStatus === "all" ? "" : requestedStatus || defaultStatus;
...
function filterBar(state, defaultCode = "", options = {}) {
  ...
  return `
    <form class="filter-shell${advancedOpen ? " is-expanded" : ""}" data-filter-form data-default-code="${safe(defaultCode)}" data-results-route="${safe(resultsRoute)}" data-location-active="${state.hasLocation ? "true" : "false"}">
      ...
      <div class="filter-primary">
        ...
        <select name="code" aria-label="Filter format">
          ${[["all", "All formats"], ["7s", "7s"], ["15s", "15s"], ["Beach", "Beach"], ["Touch", "Touch"], ["Tag", "Tag"]].map(([value, label]) => `<option value="${safe(value)}"${(!state.code && value === "all") || state.code === value ? " selected" : ""}>${safe(label)}</option>`).join("")}
        </select>
        <select name="status" aria-label="Filter status">
          ${[["all", "All dates"], ["upcoming", "Upcoming"], ["live", "Live"], ["past", "Past"]].map(([value, label]) => `<option value="${value}"${(!state.status && value === "all") || state.status === value ? " selected" : ""}>${label}</option>`).join("")}
        </select>
        <button class="button secondary location-button${state.hasLocation ? " is-active" : ""}" type="button" data-geolocate aria-label="${state.hasLocation ? "Clear location" : "Use my location"}" title="${state.hasLocation ? "Clear location" : "Use my location"}">${icons.locate}<span>${state.hasLocation ? "Clear location" : "Near me"}</span></button>
...
function bindInteractions() {
  ...
  filterForm?.querySelector("[data-geolocate]")?.addEventListener("click", (event) => {
    ...
    if (filterForm.dataset.locationActive === "true") {
      filterForm.elements.lat.value = "";
      filterForm.elements.lng.value = "";
      if (filterForm.elements.radius) filterForm.elements.radius.value = "";
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        filterForm.elements.lat.value = coords.latitude.toFixed(2);
        filterForm.elements.lng.value = coords.longitude.toFixed(2);
        applyFilters(filterForm);
      },
      (error) => {
